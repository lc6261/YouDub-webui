...
def generate_summary_txt(folder):
    """
    从summary.json生成视频摘要文本文件
    
    参数：
        folder: 包含summary.json文件的文件夹路径
    ...
    summary_json_path = os.path.join(folder, 'summary.json')
    
    if not os.path.exists(summary_json_path):
        print(f"错误: 未找到文件 {summary_json_path}")
        return
    ...
    try:
        ...
        author = summary.get("author", "未知作者")

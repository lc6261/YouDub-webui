...
输出JSON: {{"refined": "优化后的译文"}}"""
    ...
    def _quality_check(self, source, target, target_duration):
        ...
        # 调试信息
        # print(f"Debug Q: source='{source}' target='{target}' len(target)={len(target)}")
        # print(f"Debug Q: target_duration={target_duration}")
        
        # 调整阈值：允许更短的翻译结果（至少2个字符）
        if len(target) < 2:
            return False
            
        max_chars = int(target_duration * 5.0)
        # 对于很短的target_duration，适当放宽字符限制
